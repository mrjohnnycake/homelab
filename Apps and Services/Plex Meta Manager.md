...
https://mdblist.com/lists/mrjohnnycake/netflix

  -"The Langoliers" not added

- To Perfect (except for posters)
		Services
...
- Create my own genre posters, as needed
- Finish Sleep playlist



- Shows Files to Create
	- Charts
	- Countries
	- Franchises
	- Genres
	- Holidays
	- Networks
	- Specialty (sketch comedy, sci fi, etc.)
- Look into how to autocreate a yml for every media item
- I need to make note of all Plex settings, including each library setting, since that is what effected  PMM's output
- After finishing PMM yml files, go to the config file and set "save_report" to true. If you do it before then it'll cause each run to take too long which will just slow down testing.
- Look at scheduling PMM
```
pmm_scheduled:
...
https://github.com/meisnate12/Plex-Meta-Manager-Configs/blob/master/serialpotato/collections.yml
...
- Option 2: If you want to copying a pre-existing temple (like mine found below):
```
...
Paste the template into the Movies.yml file and save


## Running ##

If you only want to run it once and then quit, run this as necessary:
```
sudo docker run --rm -it -v "/docker/appdata/plex-meta-manager/config:/config:rw" meisnate12/plex-meta-manager --run
```

If you get an error where it just won't run but you're sure your setup is good, update the docker image in Portainer and then try again


To run the container all the time and have it auto run at 5a every day (that's by it's design), run this container build in the Media VM CLI (this is how I run/install it):
```d
...
## File Structure ##
	...
	config
		...
		metadata
			movies
				A League of Their Own (1992).yml
			music
			shows
		overlays
			movies
...
Oscar Winners  ("*101d*)
1001 Movies  ("*101e*)
Studios (++++++++__<<collection_name>>)
Services (+++++++__<<collection_name>>)

Countries (++++++__<<collection_name>>)

Holidays (+++++__<<collection_name>>)
Genres / Specialty (+++++__<<collection_name>>)
Franchises (++++__<<collection_name>>)

People (+++__<<collection_name>>)
...
tmdb:                                                   # REQUIRED for the script to run
  apikey: b2cb2917c2c92cef2bc180346ec7f151
  language: en
  cache_expiration: 60
  region:

trakt:
  client_id: 0f2f2ac682fbb7d4ba30eb86ecd91313c3a4311dae25b51099236aa12c1bb516
  client_secret: fa28266c8e710e82bf1ad8d76a21b2ba47938f3b413b938b5702f65e224ea8dd
  pin:
...
#   token: f21559379a044347bc31c013e8e1e6ac
#   add_missing: false
#   add_existing: false
...
#   token: 0f9e7a36432843c08e1f4753ae7d86fe
#   add_missing: false
#   add_existing: false
