...
```
http://humboldtchildcare.org/
Humboldt Child Care Stablization Fund

mailto://postfixitman:qwhrgfzoyjezlgla@gmail.com/?to=mrjohnnycake@gmail.com,annabanana2472@gmail.com
gotifys://moviesign.thecanpart.com/A3SsNe_fgdJ1_lP

Humboldt Child Care Stablization Fund - Website Updated

There has been a change to the Humboldt Child Care Stabilization Fund website!
---------------
{diff}
```
...
Notifications:
```
mailto://postfixitman:[2fa-app-password]@gmail.com/?to=mrjohnnycake@gmail.com,annabanana2472@gmail.com
gotifys://moviesign.thecanpart.com/[app-token]
```
...
```
docker run -d --name='Duplicacy' -p '3875:3875/tcp' -v '/docker/appdata/duplicacy':'/config':'rw' -v '/docker/appdata/duplicacy/cache':'/cache':'rw' -v '/docker/appdata/duplicacy/logs':'/logs':'rw' -v '/mnt/Deep-13/Duplicacy':'/restored':'rw' -e PUID=1000 -e PGID=1000 --hostname=duplicacy 'cr.hotio.dev/hotio/duplicacy'
```
...
### Optional Install on Bare Metal ###
...
services:
  gluetun:
    ...
    environment:
      ...
      - OPENVPN_USER=p9923879
      - OPENVPN_PASSWORD=SYyfjxf2t8
...
For any container that needs to use gluetun to connect, like Deluge, add this to the Stack / Compose script
...
services:
  nzbget:
    ...
    environment:
      ...
      - PGID=1000
      - TZ=America/Los_Angeles
      - NZBGET_USER=UEEWrXalrH
      - NZBGET_PASS=wLY,TRsFNo.6n3^4B9Ia
    volumes:
...
```
docker create --name='Plex' --net='media' -e TZ="America/Los_Angeles" -e 'PLEX_CLAIM'='claim-tUcFSx9QP-76ggRELExz' -e 'PLEX_UID'='99' -e 'PLEX_GID'='100' -e 'VERSION'='latest' -p '32400:32400/tcp' -v '/tmp/':'/transcode':'rw' -v '/mnt/user/data/media/':'/data/media':'rw' -v '/mnt/user/appdata/Plex-Media-Server':'/config':'rw' 'plexinc/pms-docker'
```
...
services:
  ...
  db:
    ...
    environment:
      POSTGRES_DB: wiki
      POSTGRES_PASSWORD: xkp2UCD2kdn_vcm1yke
